# Query 1
SELECT s.name, count(sac.campaign_id) AS nr_campaigns FROM sources_associated_campaigns AS sac
JOIN sources s ON s.id = sac.source_id
GROUP BY  sac.source_id
